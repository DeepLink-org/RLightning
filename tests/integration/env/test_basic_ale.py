"""This file implements test cases for basic ale tasks, which are integrated in gymnasium"""
...
from rlightning.env.ale_env import ALEEnv
...
@pytest.mark.parametrize("task", ["ALE/Adventure-v5", "ALE/Alien-v5"])
def test_env_logic(task: str):

    max_episode_steps = 10

    config = EnvConfig(name=task, task=task, backend="ale", max_episode_steps=max_episode_steps)
...
def test_feed_into_env_group(env_as_remote: bool, task: str):
